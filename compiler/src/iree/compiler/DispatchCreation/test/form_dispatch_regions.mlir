...
util.func public @fuse_no_shared_parallel_loops(%arg0: tensor<16x16x24xf32>, %arg1: tensor<64x3x32xf32>) -> (tensor<64x3x32xf32>, tensor<32x64x3xf32>) {
  ...
  %2 = tensor.empty() : tensor<32x64x3xf32>
  %3 = linalg.fill ins(%cst : f32) outs(%0 : tensor<f32>) -> tensor<f32>
  %4 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> ()>], iterator_types = ["reduction", "reduction", "reduction"]} ins(%arg0 : tensor<16x16x24xf32>) outs(%3 : tensor<f32>) {
  ^bb0(%in: f32, %out: f32):
    %6 = arith.mulf %in, %in : f32
    %7 = arith.addf %out, %6 : f32
    linalg.yield %7 : f32
  } -> tensor<f32>
  %5:2 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> ()>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d2, d0, d1)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%arg1, %4 : tensor<64x3x32xf32>, tensor<f32>) outs(%1, %2 : tensor<64x3x32xf32>, tensor<32x64x3xf32>) {
...
util.func public @reduction_elementwise_broadcast_fusion_with_unit_dims(
    %arg0: tensor<128x512x1xf32>,
    %arg1: tensor<64x1024x512x1xf32>) -> tensor<64x1024x512x1xf32> {
  %cst = arith.constant 0.000000e+00 : f32
  %0 = tensor.empty() : tensor<512x1xf32>
  %1 = linalg.fill ins(%cst : f32) outs(%0 : tensor<512x1xf32>) -> tensor<512x1xf32>
  %2 = linalg.generic {
      indexing_maps = [affine_map<(d0, d1, d2) -> (d1, d0, d2)>,
                       affine_map<(d0, d1, d2) -> (d0, d2)>],
      iterator_types = ["parallel", "reduction", "parallel"]}
      ins(%arg0 : tensor<128x512x1xf32>) outs(%1 : tensor<512x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %5 = arith.addf %in, %out : f32
      linalg.yield %5 : f32
  } -> tensor<512x1xf32>
  %3 = linalg.generic {
      indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                       affine_map<(d0, d1) -> (d0, d1)>],
      iterator_types = ["parallel", "parallel"]}
      ins(%2 : tensor<512x1xf32>) outs(%0 : tensor<512x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %5 = arith.mulf %in, %in : f32
      linalg.yield %5 : f32
  } -> tensor<512x1xf32>
  %4 = tensor.empty() : tensor<64x1024x512x1xf32>
  %result = linalg.generic {
      indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d2, d3)>,
                       affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                       affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
      iterator_types = ["parallel", "parallel", "parallel", "parallel"]}
      ins(%3, %arg1 : tensor<512x1xf32>, tensor<64x1024x512x1xf32>) outs(%4 : tensor<64x1024x512x1xf32>) {
    ^bb0(%in0: f32, %in1: f32, %out: f32):
      %5 = arith.mulf %in0, %in1 : f32
      linalg.yield %5 : f32
  } -> tensor<64x1024x512x1xf32>
  util.return %result : tensor<64x1024x512x1xf32>
}

// CHECK-LABEL: util.func public @reduction_elementwise_broadcast_fusion_with_unit_dims
//  CHECK-SAME:     %[[ARG0:[a-zA-Z0-9]+]]: tensor<128x512x1xf32>
//  CHECK-SAME:     %[[ARG1:[a-zA-Z0-9]+]]: tensor<64x1024x512x1xf32>
//       CHECK:   %[[DISPATCH0:.+]] = flow.dispatch.region
//       CHECK:     %[[REDUCTION:.+]] = linalg.generic
...
// Check that a parallel elementwise producer fuses with a full-reduction
// consumer when they share an input tensor.
util.func public @fuse_parallel_producer_with_scalar_reduction_consumer_shared_input(
    %arg0: tensor<4xf32>, %arg1: tensor<4xf32>, %init: tensor<f32>) -> tensor<f32> {
  %empty = tensor.empty() : tensor<4xf32>
  %elt = linalg.generic {
      indexing_maps = [affine_map<(d0) -> (d0)>,
                       affine_map<(d0) -> (d0)>,
                       affine_map<(d0) -> (d0)>],
      iterator_types = ["parallel"]
  } ins(%arg0, %arg1 : tensor<4xf32>, tensor<4xf32>) outs(%empty : tensor<4xf32>) {
  ...
  %sum = linalg.generic {
      indexing_maps = [affine_map<(d0) -> (d0)>,
                       ...
                       affine_map<(d0) -> ()>],
      iterator_types = ["reduction"]
  } ins(%elt, %arg1 : tensor<4xf32>, tensor<4xf32>) outs(%init : tensor<f32>) {
  ^bb0(%x: f32, %y: f32, %acc: f32):
    %0 = arith.mulf %x, %y : f32
    %1 = arith.addf %0, %acc : f32
    linalg.yield %1 : f32
  } -> tensor<f32>
  util.return %sum : tensor<f32>
}
// CHECK-LABEL: util.func public @fuse_parallel_producer_with_scalar_reduction_consumer_shared_input
//       CHECK:   %[[DISPATCH:.+]] = flow.dispatch.region
...
// Check that a parallel elementwise producer still fuses with a full-reduction
// consumer even without a shared input: the reduction only reads the producer's
// output (plus an unrelated tensor), and eliminating that intermediate tensor
// is itself the bandwidth payoff.
util.func public @fuse_parallel_producer_with_scalar_reduction_consumer_no_shared_input(
    %arg0: tensor<4xf32>, %arg1: tensor<4xf32>, %arg2: tensor<4xf32>,
  ...
  %sum = linalg.generic {
      indexing_maps = [affine_map<(d0) -> (d0)>,
                       ...
                       affine_map<(d0) -> ()>],
      iterator_types = ["reduction"]
  } ins(%elt, %arg2 : tensor<4xf32>, tensor<4xf32>) outs(%init : tensor<f32>) {
  ^bb0(%x: f32, %y: f32, %acc: f32):
    %0 = arith.mulf %x, %y : f32
    %1 = arith.addf %0, %acc : f32
    linalg.yield %1 : f32
...
// CHECK-LABEL: util.func public @fuse_parallel_producer_with_scalar_reduction_consumer_no_shared_input
//       CHECK:   %[[DISPATCH:.+]] = flow.dispatch.region
//       CHECK:     %[[ELT:.+]] = linalg.generic
//  CHECK-SAME:         iterator_types = ["parallel"]
//       CHECK:     %[[REDUCE:.+]] = linalg.generic
