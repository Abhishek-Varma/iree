...
/// Insert identity map_gather ops at the result of a LoadFromBufferOp when
/// it is followed by a relayout op chain. This is the dual of
/// InsertMapScatterOpPattern which triggers on StoreToBufferOp. The
/// `controlFn` provides control over whether or not to insert a map_gather op.
struct InsertMapGatherOpPattern : public RewritePattern {
  InsertMapGatherOpPattern(MLIRContext *context,
                           CombineRelayoutOpsControlFnRef controlFn = nullptr,
                           PatternBenefit benefit = 1)
      : RewritePattern(IREE::Codegen::LoadFromBufferOp::getOperationName(),
                       benefit, context),
        controlFn(controlFn) {}
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // LoadFromBufferOp has a single result.
    OpResult loadResult = op->getResult(0);
    // Check if the load result is used by at least one relayout op.
    bool hasRelayoutUser =
        llvm::any_of(loadResult.getUsers(), [](Operation *user) {
          return isSupportedRelayoutOp(user) && !isa<tensor::PadOp>(user);
        });
    if (!hasRelayoutUser) {
      return failure();
    }
    if (controlFn && !controlFn(loadResult)) {
      return failure();
    }
    (void)insertIdentityMapGather(rewriter, loadResult);
    return success();
  }
